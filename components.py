            ...
            if st.button("Calcular portfólio"):
                ...
                if combos:
                    ...
                    if open_combos:
                        ...
                        st.bar_chart(data=open_combos, x="Ticker", y="Ganho Nao Realizado", color="Ticker")
...
def generatePDF(total_realized,total_unrealized,total_current_value,roi_total_all, combos):
    ...
    pdf.cell(0,10,"Relatório da perfomance do portfólio")
    pdf.ln()
    ...
    pdf.cell(0,10, f"Ganhos potenciais (lucro com posições abertas) : {round(total_unrealized, 2)} EUR")
