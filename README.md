...
- No CSV, botão **“Calcular portfólio”** consolida por ticker, mostra métricas totais (total investido, valor atual, ganho total, ROI total), gráficos (barras, pizza) e a linha de ROI acumulado (usando o preço atual para todas as ordens).
- No manual, idem: consolidação, métricas, gráficos e a linha de ROI acumulado.
...
3) Escolher o modo:
   - **Calcular ativos manualmente**: definir nº de ordens, preencher Ticker, Quantidade, Preço de compra e Data. Clique “Calcular Todos”; depois, opcionalmente, “Calcular portfólio” para ver resumo consolidado, métricas totais, gráficos e a linha de ROI acumulado.
   - **Importar dados - CSV**: baixar o modelo, preencher e enviar. Clique “Calcular share-to-share” para ver cada ordem ou “Calcular portfólio” para consolidar, ver métricas totais, gráficos e a linha de ROI acumulado.
...
- In CSV mode, **“Calcular portfólio”** groups by ticker, shows portfolio totals, charts (bars, pie) and the cumulative ROI line (using today’s price applied to all orders).
- Manual mode mirrors the same totals/charts and the cumulative ROI line.
